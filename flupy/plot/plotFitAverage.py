...
def plotFitAverage(paramdict,fitdict,datadict,matrixdict):
        """
        ...
        data = datadict["average_spectrum"][irange]
        result = np.dot(DB,result_est)
        pylab.plot(x,result,label="Average of per pixel fits")
        pylab.yscale("log")
        pylab.plot(x,data,label="Average Spectrum")
        pylab.legend()
        pylab.show()


def plotFitAverageAll(paramdict,fitdict,datadict,matrixdict):
        """
        ...
        result_est=fitdict["parameters"]
        myplots = []
        print 'nel',nel_params,nel_params+nback_params
        back    = np.dot(DB[:,nel_params:nel_params+nback_params],result_est[nel_params:nel_params+nback_params])
        for j,el in enumerate(descript_el):
            if(el[1]=='xrf'):
                pylab.plot(x, back+(DB[:,j]*result_est[j]),label=el[2])
                #labels.append(el[2])
                #myplots.append(a)
            elif(el[1]=='pileup'):
                pylab.plot(x,back+(DB[:,j]*result_est[j]),label=el[2]+"_pileup")
                #labels.append(el[2]+"_pileup")
                #myplots.append(a)
            elif(el[1]=='escape'):
                pylab.plot(x, back+(DB[:,j]*result_est[j]),label=el[2]+"_escape")
...
#        ax.set_title("Average fit")
        ...
        if(nscatt_params>0):
            scatt   = np.dot(DB[:,nel_params+nback_params:nel_params+nback_params+nscatt_params],\
                            result_est[nel_params+nback_params:nel_params+nback_params+nscatt_params])
            pylab.plot(x, back+scatt)
#            labels.append("scatter")
#            myplots.append(a)
            #a,=ax.plot(x,smooth(data[fit[0],fit[1]][irange]),label="raw data")
        pylab.plot(x,data,label="raw data")
