...
def plotFitSum(paramdict,fitdict,datadict,matrixdict):
        """
        ...
        rows = datadict["rows"]
        cols = datadict["cols"]
        mysum = np.zeros_like(x)
        for r in range(rows):
            for c in range(cols):
                result = np.dot(DB,result_est[r,c])
                mysum = mysum + result
        mysum = mysum/(rows*cols)
        pylab.plot(x,mysum,label="Average of per pixel fits")
        pylab.yscale("log")
