...
class XRFModel(Model1D):

    """Build and fit a model of an XRF Signal1D.

    Parameters
    ----------
    spectrum : an XRFSpectrum (or any XRFSpectrum subclass) instance.
    ...
    def as_dictionary(self, fullcopy=True):
	    """
		Dictionary of the xraylines and background components
		"""
		
        dic = super(XRFModel, self).as_dictionary(fullcopy)
    ...
    def units_factor(self):
	    """
		Energy units are keV by default.
		Check the energy units defined by the input signal spectrum
		and return a scaling factor if appropriate
		Example:
		if units are eV returns 1000.0
		if units keV return 1.0
